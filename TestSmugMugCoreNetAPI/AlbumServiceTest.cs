...
namespace TestSmugMugCoreNetAPI
{
    ...
    public class AlbumServiceTest
    {
        ...
        ///A test for Browse
        ///</summary>
        [TestMethod(), Obsolete("Browsing no longer supported.")]
        [Ignore("deprecated from SmugMugCore.NET due to lack of SmugMug 1.3 API Support")]
        public void BrowseTest()
        {
            var core = Utility.RetrieveSmugMugCore();
            var target = new AlbumService(core); 
            var albumList = target.GetAlbumList(Array.Empty<string>());

            var testAlbum = albumList[0];
            int albumId = testAlbum.AlbumId; 
            string albumKey = testAlbum.AlbumKey; 
            target.Browse(albumId, albumKey);
        }

        /// <summary>
        ///A test for AddComment
        ///</summary>
        [TestMethod(), Obsolete("Comments no longer supported.")]
        [Ignore("deprecated method calling smugmug.albums.comments.add")]
        public void AddCommentTest()
        {
            ...
            int rating = 3; 
            Comment actual;
            actual = target.AddComment(albumId, albumKey, Array.Empty<string>(), comment, rating);

            if (actual.CommentId == 0)
                Assert.Fail("Comment was not saved.");
